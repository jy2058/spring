...
public class ProfileImgView implements View{
	...
	// 개발자는 이미지 경로를 model 객체에 "path"라는 속성으로 설정한다.
	
	// 개발자는 사용자 아이디를 model 객체에 "userId"라는 속성으로 설정한다.
	@Override
	public void render(Map<String, ?> model, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		
		//response.setHeader("Content-Disposition", "attachment; filename=profile.png");	//다운로드
